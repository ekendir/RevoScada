...
namespace RevoScada.WriteService
{
    public partial class WriteService : ServiceBase
    {
        ...
        private const string _serviceName = "RevoScadaWriteService";
        ...
        protected override void OnStart(string[] args)
        {
            ...
            try
            {
                ...
                WriteConfigurations.Instance.InitializeConfiguration(_startupConfigurationFile);
            }
            catch (Exception ex)
            {
            ...
            try
            {
                LogManager.Instance.InitializeConfiguration(WriteConfigurations.Instance.WriteServiceConfiguration.LogSettings);
        ...
        private void DoWork()
        {
            ...
            try
            {
                ...
                _cycleOperationContext.ConnectDevices((PlcType)Enum.Parse(typeof(PlcType), WriteConfigurations.Instance.WriteServiceConfiguration.PlcType));
                LogManager.Instance.Log($"Plc Connection established for all devices!", LogType.Information);
            }
            catch (Exception ex)
            {
                LogManager.Instance.Log($"InitializeConnections error {ex.Message}", LogType.Error);
            }
            ...
            LogManager.Instance.Log($"<<< Write Service Cycle Started! >>>", LogType.Information);
        ...
        {
            EventLog.WriteEntry(_serviceName, $"Abort Started! (WriteService)", EventLogEntryType.Information);
            ...
            EventLog.WriteEntry(_serviceName, $"Abort Completed! (WriteService)", EventLogEntryType.Information);
        }
    }
}
